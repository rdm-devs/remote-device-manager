...
def create_device(device: schemas.DeviceCreate, db: Session = Depends(get_db)):
    ...
    if not db_device:
        raise HTTPException(status_code=400, detail=f"Device group {device.device_group_id} not found")
    return db_device
...
def update_device(
    ...
    if not updated_device:
        raise HTTPException(status_code=400, detail="Device could not be updated")
