...
@router.post("/user_group/", response_model=schemas.UserGroup)
...
@router.get("/user_group/{group_id}", response_model=schemas.UserGroup)
...
@router.patch("/user_group/{group_id}", response_model=schemas.UserGroup)
def update_user_group(group_id: int, 
                    user_group: schemas.UserGroupUpdate, 
                    db: Session = Depends(get_db)):

    db_user_group = read_user_group(group_id, db)
    updated_user_group = service.update_user_group(db, 
                                                db_user_group, 
                                                updated_user_group=user_group)
...
@router.delete("/user_group/{group_id}", response_model=schemas.UserGroupDelete)
def delete_user_group(group_id: int, db: Session = Depends(get_db)):
    db_user_group = read_user_group(group_id, db)
    deleted_user_group = service.delete_user_group(db, db_user_group)
    if not deleted_user_group:
