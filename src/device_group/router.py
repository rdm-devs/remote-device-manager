...
def create_device_group(device_group: schemas.DeviceGroupCreate, db: Session = Depends(get_db)):
    db_device_group = service.get_device_group_by_name(db, device_group_name=device_group.name)
    if db_device_group:
...
def read_device_group(device_group_id: int, db: Session = Depends(get_db)):
    db_device_group = service.get_device_group(db, device_group_id=device_group_id)
